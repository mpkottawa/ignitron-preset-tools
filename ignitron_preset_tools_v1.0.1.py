...
def generate_chart(base_dir):
    ...
    # Build table
    header = ["Bank", "Slot 1", "Slot 2", "Slot 3", "Slot 4"]
    table_data = [header]
...
class PresetPicker(tk.Tk):
    def __init__(self):
        ...
        # Size to fit the screen horizontally (no clipping on macOS)
        sw = self.winfo_screenwidth()
        sh = self.winfo_screenheight()

        target_w = max(1100, int(sw * 0.86))
        target_w = min(target_w, sw - 40)   # never exceed screen width minus margin
        target_h = max(700, int(sh * 0.88))
    ...
    def _build_ui(self):
        ...
        # Choose a list width that leaves room for 4 slots on smaller screens
        listbox_chars = 44
        sw = self.winfo_screenwidth()
        if sw < 1440:
            listbox_chars = 36
        ...
        self.preset_list = tk.Listbox(
            left, width=listbox_chars, height=34, activestyle="none",
    ...
    def _render_banks(self):
        ...
        for b in range(1, self.bank_count + 1):
            ...
            ttk.Button(
                bank_frame, text="❌ Remove", command=lambda bb=b: self._remove_bank(bb)
            ).pack(side="right")

            # Slots grid for this bank
            slots_frame = tk.Frame(self.grid_frame, bg="#1e1e1e")
            slots_frame.pack(fill="x", pady=(0, 6))

            # ✅ Make 4 equal columns that expand evenly
            for c in range(4):
                slots_frame.grid_columnconfigure(c, weight=1, uniform="slotcols")

            # Create 4 slot labels (render actual values if present)
            for s in range(1, 5):
                current = self.slots.get((b, s))
                lbl = tk.Label(
                    slots_frame,
                    text=current if current else "[Empty]",
                    bd=1,
                    relief="groove",
                    height=2,            # no fixed width so it can auto-size
                    bg="#2d6a4f" if current else "#333333",
                    fg="#dddddd",
                    anchor="center"
                )
                lbl.grid(row=0, column=s - 1, padx=6, pady=6, sticky="nsew")

                # Slot metadata + event bindings
    ...
    def _fit_to_banks(self):
        ...
        # Height based on content, with bottom margin
        req_h = min(self.canvas.winfo_reqheight() + 200, scr_h - 100)

        # Clamp width to never exceed screen width
        cur_w = self.winfo_width()
        max_w = scr_w - 40
        new_w = min(cur_w, max_w)

        self.geometry(f"{new_w}x{req_h}")
...
def choose_port():
    ports = list(serial.tools.list_ports.comports())
    if not ports:
        print("❌ No COM ports found. Please connect your Ignitron and try again.")
        return None
...
def connect():
    port = choose_port()
    if not port:
        raise SystemExit("No valid COM port selected.")
    ser = serial.Serial(port, BAUD, timeout=0.5)
    #print(f"Connected on {ser.port} at {BAUD}")
    return ser
