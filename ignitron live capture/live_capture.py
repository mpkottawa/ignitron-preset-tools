...
"""
Ignitron Standalone Live Capture
--------------------------------
Connects to the Ignitron pedal over serial, fetches active preset list
and all presets, saves each preset as JSON into ./presets_TIMESTAMP/,
and generates:

- PresetListUUIDs_TIMESTAMP.txt (inside same folder, filename + UUID list, UUID uppercased, blanks ignored)
...
def print_summary(stats, out_dir, ts):
    ...
    print(f"   UUIDs:     {(out_dir / f'PresetListUUIDs_{ts}.txt').resolve()}")
...
def _write_preset_file(filename, buffer, out_dir, include_only):
    ...
    try:
        ...
        uuid_up = str(preset.get("UUID", "")).upper()
    except Exception:
        pass
...
def _extract_lines_to_files(lines, out_dir: Path, include_only=None):
        ...
        if low.startswith("reading preset filename:"):
            if filename and buffer:
                ...
                if base in seen_files:
                    ...
                    ok, uuid = _write_preset_file(filename, buffer, out_dir, include_only)
                    if ok:
                        saved += 1
                        if uuid:
                            uuid_map[base] = uuid
    ...
    return {"scanned": scanned, "saved": saved, "skipped": skipped, "duplicate": duplicate, "uuids": uuid_map}
...
def live_serial_capture_and_save(port: str,
    ...
    # Write PresetListUUIDs_TIMESTAMP.txt (ignore blanks, inside preset folder)
    uuid_file = out_dir / f"PresetListUUIDs_{ts}.txt"
    with open(uuid_file, "w", encoding="utf-8") as f:
        if ordered_list:  # mode 1, follow pedal order
            for fn in ordered_list:
                if fn in stats["uuids"]:
                    f.write(f"{fn} {stats['uuids'][fn]}\n")
        else:  # mode 2, dump all saved presets
            for fn, uuid in stats["uuids"].items():
                if uuid:
                    f.write(f"{fn} {uuid}\n")
